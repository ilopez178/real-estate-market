  ...
  return (
    <div className="bg-gray-50 rounded-md p-4">
      <div className="flex items-center gap-2 mb-2">
        <span className="text-[#0073B9]">{icon}</span>
        <span className="text-xs font-medium text-gray-500 uppercase tracking-wide">{label}</span>
      </div>
      <div className="text-xl font-bold text-gray-800">{value ?? <span className="text-gray-400 text-base font-normal">N/A</span>}</div>
      {date && <div className="text-xs text-gray-400 mt-1">{formatDate(date)}</div>}
...
export default function StateMarketData({ data, stateName, loading }: Props) {
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
      <h2 className="text-lg font-semibold text-gray-800 mb-4">{stateName} Market Data</h2>

      {loading ? (
        <div className="grid grid-cols-2 gap-3 animate-pulse">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-gray-100 rounded-md h-24" />
          ))}
        </div>
      ) : (
